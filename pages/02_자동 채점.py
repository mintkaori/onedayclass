...
with st.sidebar:
    ...
    # 시스템 프롬프트 (하드코딩)
    system_prompt = "당신은 초등학교 수학 선생님입니다. 사진을 업로드하면 파란색 선으로 10개씩 잘 묶었는지 확인하고 잘 묶었다면 사진의 오른쪽 위 파란색 한글의 이름을 불러주면서 10개씩 잘 묶었다고 칭찬해주세요."
...
def generate_answer(image_filepath, system_prompt, model_name="gpt-4o"):
    ...
    llm = ChatOpenAI(
        ...
        openai_api_key=st.session_state.get("api_key")  # API 키
    )
